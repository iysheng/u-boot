...
void yyfish_led_init(void)
{
    *(REG_PTR)(RCC_REG_BASE + RCC_CR) |= 1;
    *(REG_PTR)(RCC_REG_BASE + RCC_CFGR) = 0x00000000U;
    *(REG_PTR)(RCC_REG_BASE + RCC_CR) &= 0xFEF6FFFFU;
    *(REG_PTR)(RCC_REG_BASE + RCC_PLLCFGR) = 0x24003010U;
    *(REG_PTR)(RCC_REG_BASE + RCC_CR) &= 0xFFFBFFFFU;
    *(REG_PTR)(RCC_REG_BASE + RCC_CIR) = 0x00000000U;
    #if 0
    led_ptr_tmp = (REG_PTR)(RCC_REG_BASE + RCC_CR);
    *led_ptr_tmp |= (1 << 16);
    do {
        reg_tmp = *led_ptr_tmp;
        reg_tmp &= (1 << 17);
    ...
    *(REG_PTR)(RCC_REG_BASE + RCC_APB1ENR_OFFSET) |= 1<<8;
    *(REG_PTR)(PWR_REG_BASE + PWD_CR1) |= 3<<14; 		//高性能模式,时钟可到180Mhz
	*(REG_PTR)(PWR_REG_BASE + PWD_CR1) |= 1<<16; 		//使能过驱动,频率可到216Mhz
	*(REG_PTR)(PWR_REG_BASE + PWD_CR1) |= 1<<17; 		//使能过驱动切换
	*(REG_PTR)(RCC_REG_BASE + RCC_CFGR) |= (0<<4)|(5<<10)|(4<<13);//HCLK 不分频;APB1 4分频;APB2 2分频. 
	*(REG_PTR)(RCC_REG_BASE + RCC_CR) &= ~(1<<24);		//关闭主PLL
	*(REG_PTR)(RCC_REG_BASE + RCC_PLLCFGR) = 25|(432<<6)|(((2>>1)-1)<<16)|(9<<24)|(1<<22);//配置主PLL,PLL时钟源来自HSE
	*(REG_PTR)(RCC_REG_BASE + RCC_CR) |= 1<<24;			//打开主PLL
	while((*(REG_PTR)(RCC_REG_BASE + RCC_CR) & (1<<25)) == 0);//等待PLL准备好  
	*(REG_PTR)(RCC_REG_BASE + RCC_CFGR) &= ~(3<<0);		//清零
	*(REG_PTR)(RCC_REG_BASE + RCC_CFGR) |= 2<<0;		//选择主PLL作为系统时钟	 
	while((*(REG_PTR)(RCC_REG_BASE + RCC_CFGR) & (3<<2)) != (2<<2));//等待主PLL作为系统时钟成功.
    #endif    
    *(REG_PTR)(RCC_REG_BASE + RCC_AHB1ENR_OFFSET) |=  (0x1 << 8);
    *(REG_PTR)GPIO_I_REG_BASE &=  ~(0x3 << 22);
    *(REG_PTR)GPIO_I_REG_BASE |=  (0x1 << 22);
    *(REG_PTR)(GPIO_I_REG_BASE + 0x0CU) &=  ~(0x3 << 22);
    *(REG_PTR)(GPIO_I_REG_BASE + 0x0CU) |=  (0x1 << 22);//output pull up mode
    *(REG_PTR)(GPIO_I_REG_BASE + 0x14U) &= ~(1 << 11);//default low value
}
